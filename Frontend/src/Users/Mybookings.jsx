...
function Mybookings() {
  ...
  useEffect(() => {
    ...
    if (user) {
      axios.get(`http://localhost:7000/user/getbookings/${user.id}`)
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 20px' }}>
        ...
        {items.map((item) => (
          <div key={item._id} ref={selectedCard?._id === item._id ? pdref : null} style={{
            ...
            <img src={`http://localhost:7000/organizer/${item.templeImage}`} alt="Temple" style={{ width: '80px', height: '80px', borderRadius: '12px', objectFit: 'cover' }} />
