...
const Usignup = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post("http://localhost:7000/user/usignup", { name, email, password })
      .then(() => { alert('Account created'); navigate('/ulogin'); })
    ...
    <div style={{ minHeight: '100vh', background: '#0f0f23', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      ...
      <div style={{ width: '100%', maxWidth: '420px', background: 'rgba(26, 26, 46, 0.8)', border: '1px solid rgba(249, 115, 22, 0.15)', borderRadius: '24px', padding: '48px 36px', backdropFilter: 'blur(20px)', boxShadow: '0 20px 60px rgba(0, 0, 0, 0.4)' }}>
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <h2 style={{ fontSize: '1.8rem', fontWeight: '700', marginTop: '12px', background: 'linear-gradient(135deg, #f97316, #f59e0b)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Create Account</h2>
          <p style={{ color: '#64748b', fontSize: '0.9rem', marginTop: '4px' }}>Join DarshanEase today</p>
        </div>
        ...
        <p style={{ textAlign: 'center', marginTop: '20px', color: '#64748b', fontSize: '0.9rem' }}>
          Already have an account? <Link to="/ulogin" style={{ color: '#f97316', fontWeight: '600' }}>Login</Link>
        </p>
      </div>
    </div>
  );
