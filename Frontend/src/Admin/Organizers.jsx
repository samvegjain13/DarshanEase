  ...
  return (
    <div style={{ background: '#0f0f23', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 20px' }}>
        <h2 style={{ textAlign: 'center', fontSize: '2rem', fontWeight: '700', background: 'linear-gradient(135deg, #f97316, #f59e0b)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', marginBottom: '32px' }}>Organizers</h2>
              ...
              {users.map((item, index) => (
                <tr key={item._id} style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.03)' }}>
                  ...
                  <td style={{ padding: '12px 16px', display: 'flex', gap: '8px' }}>
                    <Link to={`/organizeredit/${item._id}`} style={{ padding: '6px 12px', background: 'rgba(59, 130, 246, 0.15)', color: '#60a5fa', border: '1px solid rgba(59, 130, 246, 0.3)', borderRadius: '8px', fontSize: '0.8rem', textDecoration: 'none' }}>✏ Edit</Link>
                    ...
                    <button onClick={() => fetchOrganizerTemples(item._id)} style={{ padding: '6px 12px', background: 'rgba(249, 115, 22, 0.15)', color: '#f97316', border: '1px solid rgba(249, 115, 22, 0.3)', borderRadius: '8px', fontSize: '0.8rem', cursor: 'pointer' }}>👁 View</button>
                  </td>
                </tr>
              ))}
            </tbody>
            ...
            {userbookings.map((item) => (
              <div key={item._id} style={{ background: 'rgba(15, 15, 35, 0.8)', borderRadius: '12px', padding: '16px', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap', border: '1px solid rgba(249, 115, 22, 0.08)' }}>
                ...
                <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '8px' }}>
                  ...
                  <div><p style={{ color: '#64748b', fontSize: '0.7rem', margin: 0 }}>Address</p><p style={{ color: '#f1f5f9', fontSize: '0.85rem', margin: 0 }}>{item.location}</p></div>
                </div>
                <button onClick={() => deleteitem(item._id)} style={{ padding: '6px 12px', background: 'rgba(239, 68, 68, 0.15)', color: '#f87171', border: '1px solid rgba(239, 68, 68, 0.3)', borderRadius: '8px', fontSize: '0.75rem', cursor: 'pointer' }}>🗑</button>
              </div>
            ))}
